...
let textures = {};
let spawnInterval1 = null, spawnInterval2 = null;
let animFrame1 = null, animFrame2 = null;
let lastTimestamp1 = 0, lastTimestamp2 = 0;

let area1, area2, timerDisplay, name1, score1, name2, score2, message1, message2;
let gameContainer;
...
async function authenticate(password) {
    const resp = await fetch('/board/auth', {
        method: 'POST',
    ...
    if (resp.ok) {
        ...
        document.getElementById('authError').innerText = err.error || 'Неверный пароль';
        return false;
    }
}

document.getElementById('authSubmitBtn').addEventListener('click', () => {
    const pwd = document.getElementById('boardPassword').value;
    authenticate(pwd);
});
...
function startGameLoop() {
    area1 = document.getElementById('gameArea1');
    area2 = document.getElementById('gameArea2');
    timerDisplay = document.getElementById('timerDisplay');
    name1 = document.getElementById('teamNameDisplay1');
    score1 = document.getElementById('scoreDisplay1');
    name2 = document.getElementById('teamNameDisplay2');
    score2 = document.getElementById('scoreDisplay2');
    message1 = document.getElementById('messageOverlay1');
    message2 = document.getElementById('messageOverlay2');
        ...
        for (var i = list.length - 1; i >= 0; i--) {
            ...
            if (Math.abs(clickX - cx) <= rx && Math.abs(clickY - cy) <= ry) {
                ...
                if (enemy.life <= 0) {
                    ...
                    let sendTeamId;
                    let currentMode = document.getElementById('gameArea1').style.display === 'block' && document.getElementById('gameArea2').style.display === 'block' ? '2board4team' : '2board2team';
                    if (currentMode === '2board2team') sendTeamId = '2';
                    else if (currentMode === '2board4team') sendTeamId = teamId === '1' ? '3' : '4';
                    else sendTeamId = '1';
                    addScore(sendTeamId, points);
